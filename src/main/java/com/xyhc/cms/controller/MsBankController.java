 ...
 */
@RestController
@RequestMapping("/msBank")
public class MsBankController {
    @Autowired
     ...
     */
    @RequestMapping("/all")
    public List<MsBankEntity> all(@RequestParam Map<String, Object> params) {
        return msBankService.all(params);
    }
     ...
     */
    @GetMapping("/bankList")
    public PageUtils bankList(@RequestParam Map<String, Object> params) {
        try {
            return msBankService.bankList(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(msBankService.detail(id));
        } catch (Exception e) {
    ...
    public Result saveBank(@RequestBody MsBankEntity msBank) {
        try {
            msBankService.saveBank(msBank);
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return msBankService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<MsBankEntity> msBankEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            msBankEntity = msBankService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), MsBankEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(msBankEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
