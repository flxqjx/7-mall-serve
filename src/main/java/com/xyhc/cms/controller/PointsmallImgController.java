 ...
 */
@RestController
@RequestMapping("/pointsmallImg")
public class PointsmallImgController {
    @Autowired
    private PointsmallImgService pointsmallImgService;
     ...
     */
    @RequestMapping("/all")
    public List<PointsmallImgEntity> all(@RequestParam Map<String, Object> params) {
        return pointsmallImgService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return pointsmallImgService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(pointsmallImgService.detail(id));
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody PointsmallImgEntity pointsmallImg) {
        try {
            pointsmallImgService.save(pointsmallImg);
    ...
    try {
            pointsmallImgService.remove(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    try {
            pointsmallImgService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return pointsmallImgService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<PointsmallImgEntity> pointsmallImgEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            pointsmallImgEntity = pointsmallImgService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), PointsmallImgEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(pointsmallImgEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
