 ...
 */
@RestController
@RequestMapping("/commodityShoppingCart")
public class CommodityShoppingCartController {
     ...
     */
    @RequestMapping("/all")
    public List<CommodityShoppingCartEntity> all(@RequestParam Map<String, Object> params) {
        return commodityShoppingCartService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return commodityShoppingCartService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(commodityShoppingCartService.detail(id));
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody CommodityShoppingCartEntity commodityShoppingCart) {
        try {
            commodityShoppingCartService.save(commodityShoppingCart);
    ...
    try {
            commodityShoppingCartService.remove(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    try {
            commodityShoppingCartService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return commodityShoppingCartService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<CommodityShoppingCartEntity> commodityShoppingCartEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            commodityShoppingCartEntity = commodityShoppingCartService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), CommodityShoppingCartEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(commodityShoppingCartEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
