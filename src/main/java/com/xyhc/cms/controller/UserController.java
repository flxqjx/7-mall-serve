...
public class UserController {
     ...
     */
    @RequestMapping("/all")
    public List<UserEntity> all(@RequestParam Map<String, Object> params) {
    ...
    public Result userInfo() {
        try {
            return Result.success(userService.detail(authUtils.AuthUser().getUserId()));
        } catch (Exception e) {
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        ...
        if (params != null && params.size() > 0) {
            userEntity = userService.all(params);
