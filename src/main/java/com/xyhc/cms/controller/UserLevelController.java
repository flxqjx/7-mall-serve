...
public class UserLevelController {
     ...
     */
    @RequestMapping("/all")
    public List<UserLevelEntity> all(@RequestParam Map<String, Object> params) {
        return userLevelService.all(params);
    }
    ...
    public PageUtils list(@RequestParam Map<String, Object> params) {
        try {
            return userLevelService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(userLevelService.detail(id));
        } catch (Exception e) {
    ...
    try {
            userLevelService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return userLevelService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<UserLevelEntity> userLevelEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            userLevelEntity = userLevelService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), UserLevelEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(userLevelEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
