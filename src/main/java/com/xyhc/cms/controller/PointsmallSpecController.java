 ...
 */
@RestController
@RequestMapping("/pointsmallSpec")
public class PointsmallSpecController {
    @Autowired
    private PointsmallSpecService pointsmallSpecService;
     ...
     */
    @RequestMapping("/all")
    public List<PointsmallSpecEntity> all(@RequestParam Map<String, Object> params) {
        return pointsmallSpecService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return pointsmallSpecService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public  Result detail(@RequestParam String id) {
        try {
            return Result.success(pointsmallSpecService.detail(id));
        } catch (Exception e) {
    ...
    public  Result detailbyid(@RequestParam String id) {
        try {
            return Result.success(pointsmallSpecService.detailbyid(id));
        } catch (Exception e) {
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody PointsmallSpecEntity pointsmallSpec) {
        try {
            pointsmallSpecService.save(pointsmallSpec);
    ...
    try {
            pointsmallSpecService.remove(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    try {
            pointsmallSpecService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return pointsmallSpecService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<PointsmallSpecEntity> pointsmallSpecEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            pointsmallSpecEntity = pointsmallSpecService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), PointsmallSpecEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(pointsmallSpecEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
