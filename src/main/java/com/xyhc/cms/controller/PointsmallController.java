...
public class PointsmallController {
     ...
     */
    @RequestMapping("/all")
    public List<PointsmallEntity> all(@RequestParam Map<String, Object> params) {
        return pointsmallService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return pointsmallService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(pointsmallService.detail(id));
        } catch (Exception e) {
    ...
    /*public Result save(@RequestBody PointsmallEntity pointsmall) { */


    @PostMapping("/save")
    public Result save(@RequestBody PointsmallImgDto pointsmallSave) {
        try {
            return pointsmallService.save(pointsmallSave);
        } catch (Exception ex) {
    ...
    try {
            pointsmallService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return pointsmallService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<PointsmallEntity> pointsmallEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            pointsmallEntity = pointsmallService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), PointsmallEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(pointsmallEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
