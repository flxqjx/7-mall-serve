...
public class CommodityCommentReplyController {
     ...
     */
    @RequestMapping("/all")
    public List<CommodityCommentReplyEntity> all(@RequestParam Map<String, Object> params) {
        return commodityCommentReplyService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return commodityCommentReplyService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(commodityCommentReplyService.detail(id));
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody CommodityCommentReplyEntity commodityCommentReply) {
        try {
            commodityCommentReplyService.save(commodityCommentReply);
    ...
    try {
            commodityCommentReplyService.remove(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    try {
            commodityCommentReplyService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return commodityCommentReplyService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<CommodityCommentReplyEntity> commodityCommentReplyEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            commodityCommentReplyEntity = commodityCommentReplyService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), CommodityCommentReplyEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(commodityCommentReplyEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
