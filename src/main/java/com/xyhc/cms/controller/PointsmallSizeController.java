 ...
 */
@RestController
@RequestMapping("/pointsmallSize")
public class PointsmallSizeController {
    @Autowired
    private PointsmallSizeService pointsmallSizeService;
     ...
     */
    @RequestMapping("/all")
    public List<PointsmallSizeEntity> all(@RequestParam Map<String, Object> params) {
        return pointsmallSizeService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return pointsmallSizeService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(pointsmallSizeService.detail(id));
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody PointsmallSizeEntity pointsmallSize) {
        try {
            pointsmallSizeService.save(pointsmallSize);
    ...
    try {
            pointsmallSizeService.remove(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    try {
            pointsmallSizeService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return pointsmallSizeService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<PointsmallSizeEntity> pointsmallSizeEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            pointsmallSizeEntity = pointsmallSizeService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), PointsmallSizeEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(pointsmallSizeEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
