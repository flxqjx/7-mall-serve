 ...
 */
@RestController
@RequestMapping("/pointsmallShoppingCart")
public class PointsmallShoppingCartController {
     ...
     */
    @RequestMapping("/all")
    public List<PointsmallShoppingCartEntity> all(@RequestParam Map<String, Object> params) {
        return pointsmallShoppingCartService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return pointsmallShoppingCartService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(pointsmallShoppingCartService.detail(id));
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody PointsmallShoppingCartEntity pointsmallShoppingCart) {
        try {
            pointsmallShoppingCartService.save(pointsmallShoppingCart);
    ...
    try {
            pointsmallShoppingCartService.remove(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    try {
            pointsmallShoppingCartService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return pointsmallShoppingCartService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<PointsmallShoppingCartEntity> pointsmallShoppingCartEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            pointsmallShoppingCartEntity = pointsmallShoppingCartService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), PointsmallShoppingCartEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(pointsmallShoppingCartEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
