...
public class TimeLimitController {
     ...
     */
    @RequestMapping("/all")
    public List<TimeLimitEntity> all(@RequestParam Map<String, Object> params) {
        return timeLimitService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return timeLimitService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(timeLimitService.detail(id));
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody TimeLimitEntity timeLimit) {
        try {
    ...
    public Result updateTime(@RequestParam String id) {
        try {
            timeLimitService.updateTime(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result removeBatch(@RequestBody String[] ids) {
        try {
            timeLimitService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
        return timeLimitService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<TimeLimitEntity> timeLimitEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            timeLimitEntity = timeLimitService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), TimeLimitEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(timeLimitEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
