 ...
 */
@RestController
@RequestMapping("/commoditySize")
public class CommoditySizeController {
    @Autowired
    private CommoditySizeService commoditySizeService;
     ...
     */
    @RequestMapping("/all")
    public List<CommoditySizeEntity> all(@RequestParam Map<String, Object> params) {
        return commoditySizeService.all(params);
    }
    ...
    public PageUtils page(@RequestParam Map<String, Object> params) {
        try {
            return commoditySizeService.page(params);
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public Result detail(@RequestParam String id) {
        try {
            return Result.success(commoditySizeService.detail(id));
        } catch (Exception e) {
     ...
     */
    @RequestMapping("/save")
    public Result save(@RequestBody CommoditySizeEntity commoditySize) {
        try {
            commoditySizeService.save(commoditySize);
    ...
    try {
            commoditySizeService.remove(id);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    try {
            commoditySizeService.removeBatch(ids);
            return Result.success();
        } catch (Exception ex) {
            return Result.error(ex.getMessage());
        }
    ...
    public Result upload(@RequestParam("Files") MultipartFile file) throws IOException {
          return commoditySizeService.upload(file);
    }
    ...
    public void export(HttpServletResponse response, @RequestParam Map<String, Object> params) throws IOException {
        List<CommoditySizeEntity> commoditySizeEntity = new ArrayList();

        if (params != null && params.size() > 0) {
            commoditySizeEntity = commoditySizeService.all(params);
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
            String fileName = URLEncoder.encode("数据导出", "UTF-8");
            response.setHeader("Content-disposition", "attachment;filename=" + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".xlsx");

            EasyExcel.write(response.getOutputStream(), CommoditySizeEntity.class)
                    .sheet("sheet1")
                    .registerWriteHandler(defaultStyles())
                    .doWrite(commoditySizeEntity);
        } else {
            response.setContentType("multipart/form-data");
            response.setCharacterEncoding("utf-8");
